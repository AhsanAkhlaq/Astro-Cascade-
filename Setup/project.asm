...
mfloop1:
	...
	mfloop2:
		...
		cmp word[es:esi],ax
		jne mfskip2
			mov word[es:esi],0x0720
			cmp word[es:esi+160],0x01db; my ship body
			je mfskip21
			cmp word[es:esi+160],0x049d;my ship 
			je mfskip21
			cmp word[es:esi+160],0x02cd ; lower screen boundary
			je mfskip2
			mov word[es:esi+160],ax
			jmp mfskip2
...
mov_boss_atk_3:
	...
	mov ax,[enemy]
	mov al,0xdc
	cmp word[es:esi],ax; left atk movement
	je mbatemmp1
		mbatemmp0:
			jmp mfskip40
		mbatemmp1:
		...
		mov word[es:esi],0x0720
		cmp word[es:esi+156],0x01db; my ship body
		je mfskip41
		cmp word[es:esi+156],0x049d;my ship 
		je mfskip41
		cmp word[es:esi+156],0x02cd ; lower screen boundary
		je mfskip40
		cmp word[es:esi+156],0x02c8 ; lower left corner screen boundary
		je mfskip40
		cmp word[es:esi+158],0x02c8 ; lower screen boundary
		je mfskip40
			cmp word[es:esi+158],0x02ba;side  screen boundary
			jne mfskip42
				mov al,0xdf
				mov word[es:esi+164],ax
			jmp mfskip40
			mfskip42:
			cmp word[es:esi+156],0x02ba;side  screen boundary
			jne mfskip43
				mov al,0xdf
				mov word[es:esi+164],ax
			jmp mfskip40
			mfskip43:
				mov word[es:esi+156],ax
		jmp mfskip40
...
mfskip40:
	...
	mov al,0xdf
	cmp word[es:esi],ax; right atk movement
	je mbatemmp3
		mbatemmp2:
			jmp mfskip50
		mbatemmp3:
		...
		mov word[es:esi],0x0720
		cmp word[es:esi+164],0x01db; my ship body
		je mfskip51
		cmp word[es:esi+164],0x049d;my ship 
		je mfskip51
		cmp word[es:esi+164],0x02cd ; lower screen boundary
		je mfskip50
		cmp word[es:esi+162],0x02bc ; lower screen boundary
		je mfskip50
		cmp word[es:esi+164],0x02bc ; lower screen boundary
		je mfskip50
			cmp word[es:esi+162],0x02ba;side  screen boundary
			jne mfskip52
				mov al,0xdc
				mov word[es:esi+156],ax
			jmp mfskip50
			mfskip52:
			cmp word[es:esi+164],0x02ba;side  screen boundary
			jne mfskip53
				mov al,0xdc
				mov word[es:esi+156],ax
			jmp mfskip50
